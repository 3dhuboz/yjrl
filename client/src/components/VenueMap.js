...
import { mapsUrl, googlePlaceEmbed } from '../../../shared/maps';
...
export default function VenueMap({ venue, url, embedUrl, compact = false }) {
  ...
  const config = useMapsConfig(!compact && !embed && !!link && link.includes('query_place_id='));
  if (!embed && config?.embedKey) embed = googlePlaceEmbed(link, config.embedKey);
