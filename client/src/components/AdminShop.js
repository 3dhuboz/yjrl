...
export default function AdminShop({ players }) {
  ...
  return <div>
    <div className="admin-toolbar" style={{ justifyContent: 'space-between' }}><h2>Uniforms & merchandise</h2><button className="yjrl-btn yjrl-btn-primary" onClick={() => edit({ id: crypto.randomUUID(), name: '', category: 'uniform', description: '', priceCents: 0, options: ['One size'], image: '', available: false, published: false })}><Plus size={16} /> Add Product</button></div>
    ...
    <form className="yjrl-card" style={{ padding: 20, margin: '20px 0' }} onSubmit={saveSettings}>
      <h3>Shop setup</h3><p>Add Nathan’s catalogue, collection details and policies before opening orders. Availability is managed by the club for each product.</p>
    ...
    <div className="yjrl-grid-3">{data.products.map(product => <div key={product.id} className="yjrl-card" style={{ padding: 20 }}>{product.image && <img src={product.image} alt={product.name} style={{ width: '100%', height: 150, objectFit: 'contain' }} />}<h3>{product.name}</h3><p>{money(product.priceCents)} · {product.published ? 'Published' : 'Draft'} · {product.available ? 'Available to order' : 'Unavailable'}</p><p>{product.options.join(', ')}</p><div className="admin-toolbar"><button className="yjrl-btn yjrl-btn-secondary" onClick={() => edit(product)}>Edit Product</button><button className="yjrl-btn yjrl-btn-danger" onClick={async () => { if (!window.confirm(`Remove ${product.name} from the shop?`)) return; try { await api.delete(`/yjrl/shop/products/${product.id}`); setData(prev => ({ ...prev, products: prev.products.filter(item => item.id !== product.id) })); } catch { setError('Could not remove product'); } }}>Remove</button></div></div>)}</div>
    ...
    {form && <div className="yjrl-modal-overlay"><form className="yjrl-modal" onSubmit={saveProduct}>
      ...
      <div className="yjrl-modal-body">{error && <p role="alert" className="yjrl-form-error">{error}</p>}
        ...
        <label>Sizes / colours (one option per line)<textarea aria-label="Sizes / colours" required className="yjrl-input" rows={4} value={form.optionText} placeholder="Size 10 / Blue&#10;Size 12 / Blue" onChange={e => setForm({ ...form, optionText: e.target.value })} /></label>
