...
const inStock = (product, option) => product?.stock?.find(item => item.option === option)?.available || 0;
const selectedSize = (product, options) => options[product.id] || product.options.find(option => inStock(product, option) > 0) || product.options[0];
...
export default function YJRLShop() {
  ...
  const unavailable = rows.some(item => !item.product?.available || !item.product.options.includes(item.option) || !Number.isInteger(item.quantity) || item.quantity < 1 || item.quantity > Math.min(20, inStock(item.product, item.option)));
  const pay = async id => {
  ...
  const checkout = async e => {
    ...
    } catch (e) { setError(e.response?.data?.error || 'Your order could not be confirmed. Please retry with this basket.'); if (e.response?.data?.code === 'stock_changed') { try { setData((await api.get('/yjrl/shop')).data); } catch { /* Keep the order error. */ } } }
    finally { setBusy(false); }
  };
  return <YJRLLayout>
    ...
    <div style={{ maxWidth: 1280, margin: 'auto', padding: '2rem 1.5rem' }}>
      ...
      {data && <div className="yjrl-grid-3">{data.products.map(product => <article className="yjrl-card" style={{ padding: 20 }} key={product.id}>{product.image ? <img src={product.image} alt={product.name} style={{ width: '100%', height: 220, objectFit: 'contain' }} /> : <div style={{ background: '#f1f5f9', borderRadius: 10, padding: 45, textAlign: 'center', color: '#64748b' }}><ShoppingBag size={48} /></div>}<p style={{ color: 'var(--yjrl-muted)', textTransform: 'capitalize' }}>{product.category}</p><h2>{product.name}</h2><p>{product.description}</p><strong>{money(product.priceCents)}</strong><label style={{ display: 'block', margin: '12px 0' }}>Size / colour<select aria-label={`Size / colour for ${product.name}`} className="yjrl-input" value={selectedSize(product, options)} onChange={e => setOptions({ ...options, [product.id]: e.target.value })}>{product.options.map(option => <option key={option} value={option} disabled={inStock(product, option) < 1}>{option}{inStock(product, option) < 1 ? " — Out of stock" : ""}</option>)}</select></label><p>{inStock(product, selectedSize(product, options))} available for this size</p><button className="yjrl-btn yjrl-btn-primary" disabled={!data.settings.ordersOpen || !product.available || busy || inStock(product, selectedSize(product, options)) <= (cart.find(item => item.productId === product.id && item.option === selectedSize(product, options))?.quantity || 0)} onClick={() => {
        const option = selectedSize(product, options), index = cart.findIndex(item => item.productId === product.id && item.option === option);
        changeCart(index < 0 ? [...cart, { productId: product.id, option, quantity: 1 }] : cart.map((item, i) => i === index ? { ...item, quantity: Math.min(20, inStock(product, option), item.quantity + 1) } : item));
      }}>{!product.available ? 'Currently unavailable' : inStock(product, selectedSize(product, options)) < 1 ? 'Out of stock' : data.settings.ordersOpen ? 'Add to Basket' : 'Orders opening soon'}</button></article>)}</div>}
      ...
      {data && cart.length > 0 && <form className="yjrl-card" onSubmit={checkout} style={{ padding: 24, marginTop: 24 }}><h2>Your basket</h2>
        {rows.map((item, index) => <div className="admin-toolbar" style={{ justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #dbe4ef' }} key={`${item.productId}-${item.option}`}><span>{item.product?.name || 'Unavailable product'} — {item.option}</span><div className="admin-toolbar"><input aria-label={`Quantity for ${item.product?.name || 'item'}`} type="number" min="1" max={Math.max(1, Math.min(20, inStock(item.product, item.option)))} disabled={busy} style={{ width: 75 }} className="yjrl-input" value={item.quantity} onChange={e => changeCart(cart.map((row, i) => i === index ? { ...row, quantity: Number(e.target.value) } : row))} /><span>{money((item.product?.priceCents || 0) * item.quantity)}</span><button type="button" aria-label={`Remove ${item.product?.name || 'item'}`} className="yjrl-btn yjrl-btn-secondary" disabled={busy} onClick={() => changeCart(cart.filter((_, i) => i !== index))}><Trash2 size={16} /></button></div></div>)}
        <h3>Total: {money(total)} AUD</h3>{unavailable && <p role="alert">Reduce quantities to the stock available, or remove unavailable items before placing your order.</p>}
