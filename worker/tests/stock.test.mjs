...
async function setup(t) {
  ...
  const order = async () => { for (const option of product.options) if (!env.DB.sqlite.prepare('SELECT 1 FROM shop_stock WHERE product_id = ? AND option = ?').get(id, option)) await count(option, 50); return (await request(shop, env, parent, '/orders', 'POST', { requestId: crypto.randomUUID(), paymentMethod: 'collection', name: 'Test parent', phone: '0400000000', acceptPolicies: true, expectedTotalCents: 7500, items: [{ productId: id, option: 'Youth 10', quantity: 2 }, { productId: id, option: "Women's 12", quantity: 1 }] })).json(); };
...
test('collection deductions are atomic across sizes and cannot run twice or lose concurrent stocktakes', async t => {
  ...
  await count('Youth 10', 10); await count("Women's 12", 5);
  const receipt = await order(); await count("Women's 12", 0, 1);
